...
requests may land you in a big issue or conflict in the codebase. So, address and merge them at earliest. If the request is way too old to be addressed, tell the person who made the pull request to raise it again from a new branch. 

git command to raise a pull request :
```bash
git push origin branch-name
```
This will push the changes into your forked repository.
